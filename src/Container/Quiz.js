...
class Quiz extends Component {
  ...
  nextQuestionHandler = () => {
    if (this.state.selectedQuestion + 1 !== this.state.question.length) {
      this.setState({ selectedQuestion: this.state.selectedQuestion + 1 });
      if (
        this.state.selectedAnswer ===
        this.state.question[this.state.selectedQuestion].correct_answer
      ) {
      ...
      this.setState({ finalScoreShow: true });
    }
  };

  answerSelectHanddler = abc => {
    console.log(abc);

    this.setState({ selectedAnswer: abc });

    // if (
    //   abc === this.state.question[this.state.selectedQuestion].correct_answer
    // ) {
    //   this.setState({ correctAnswers: this.state.correctAnswers + 1 });
    // } else {
    //   this.setState({ wrongAnswers: this.state.wrongAnswers + 1 });
  ...
  render() {
    ...
    var Question = this.state.question[this.state.selectedQuestion];

    var progress =
      ((this.state.selectedQuestion + 1) / this.state.question.length) * 100;

    var score =
      (this.state.correctAnswers / (this.state.selectedQuestion + 1)) * 100;

    score = (Math.round(score * 100) / 100).toFixed(2);

    return (
      <div>
        <Modal isOpen={this.state.finalScoreShow}>
          <ModalHeader>Your Final Score</ModalHeader>
          <ModalBody>{score} %</ModalBody>
        ...
        <Qbox
          ...
          answerSelectHanddler={this.answerSelectHanddler}
        />
        <ProgressBar>
